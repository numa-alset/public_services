...
const Map6 = () => {
    ...
    useEffect(() => {
        ...
        const Popup = ({ routeName, routeNumber, city, type, imagee, rate, lan }) => (
            <div className="popup">
                ...
                <div className="route-metric-row">
                    <h4 className="row-title" >description</h4>
                    <div className="row-value">{city}</div>
                </div>
                <div className="route-metric-row">
                    <h4 className="row-title">number</h4>
                    <div className="row-value">{type}</div>
                </div>
                <p className="route-city">your rate {rate}</p>
                <button style={{
        ...
        map2.on("load", async function () {
            ...
            //         const geoJson=data;
            //     });
            const geoJson = await axios.get("https://my-json-server.typicode.com/numa-alset/jsonph/db").then(response => response.data)
                .then((data) => {
                    return data;
                });
            map2.loadImage(
